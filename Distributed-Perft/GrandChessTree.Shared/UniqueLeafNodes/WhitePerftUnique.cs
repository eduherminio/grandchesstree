...
public partial struct Board
{
    ...
    private unsafe void AccumulateWhiteUniqueLeafNodes(int depth)
    {
        if (depth == 0)
        {
            var hash = Hash;
        ...
        var numCheckers = (byte)ulong.PopCount(checkers);
         ...
         AccumulateWhiteKingUniqueLeafNodes(depth, numCheckers > 0);
        ...
        while (positions != 0)
        {
            var index = positions.PopLSB();
             AccumulateWhitePawnUniqueLeafNodes(depth, index, AttackTables.GetRayToEdgeStraight(WhiteKingPos, index), AttackTables.GetRayToEdgeDiagonal(WhiteKingPos, index));
        }
        ...
        {
            var index = positions.PopLSB();
             AccumulateWhitePawnUniqueLeafNodes(depth, index, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF);
        }

        positions = White & Knight & ~pinMask;
        while (positions != 0)
        {
             AccumulateWhiteKnightUniqueLeafNodes(depth, positions.PopLSB());
        }
        ...
        {
            var index = positions.PopLSB();
             AccumulateWhiteBishopUniqueLeafNodes(depth, index, AttackTables.GetRayToEdgeDiagonal(WhiteKingPos, index));
        }
        ...
        {
            var index = positions.PopLSB();
             AccumulateWhiteBishopUniqueLeafNodes(depth, index, 0xFFFFFFFFFFFFFFFF);
        }
        ...
        {
            var index = positions.PopLSB();
             AccumulateWhiteRookUniqueLeafNodes(depth, index, AttackTables.GetRayToEdgeStraight(WhiteKingPos, index));
        }
        positions = White & Rook & ~pinMask;
        while (positions != 0)
        {
            var index = positions.PopLSB();
             AccumulateWhiteRookUniqueLeafNodes(depth, index, 0xFFFFFFFFFFFFFFFF);
        }
        ...
        {
            var index = positions.PopLSB();
             AccumulateWhiteQueenUniqueLeafNodes(depth, index, AttackTables.GetRayToEdgeDiagonal(WhiteKingPos, index) | AttackTables.GetRayToEdgeStraight(WhiteKingPos, index));
        }
        ...
        {
            var index = positions.PopLSB();
             AccumulateWhiteQueenUniqueLeafNodes(depth, index, 0xFFFFFFFFFFFFFFFF);
        }
    }

    public unsafe void AccumulateWhitePawnUniqueLeafNodes(int depth, int index, ulong pushPinMask, ulong capturePinMask)
    {
        Board newBoard ;
        ...
        if (rankIndex.IsSeventhRank())
        {
            ...
            while (validMoves != 0)
            {
                ...
                newBoard.WhitePawn_Capture_KnightPromotion(index, toSquare);
                 newBoard.AccumulateBlackUniqueLeafNodes(depth - 1);
                newBoard = Unsafe.As<Board, Board>(ref this);
...
// Loop over each destination square (each set bit in validMoves)
    ...
    public unsafe void AccumulateWhiteKnightUniqueLeafNodes(int depth, int index)
    {
    ...
    public unsafe void AccumulateWhiteBishopUniqueLeafNodes(int depth, int index, ulong pinMask)
    {
    ...
    public unsafe void AccumulateWhiteRookUniqueLeafNodes(int depth, int index, ulong pinMask)
    {
    ...
    public unsafe void AccumulateWhiteQueenUniqueLeafNodes(int depth, int index, ulong pinMask)
    {
    ...
    public unsafe void AccumulateWhiteKingUniqueLeafNodes(int depth, bool inCheck)
    {
