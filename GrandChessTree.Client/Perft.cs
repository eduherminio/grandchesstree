       ...
       public static void PerftRoot(ref Board board, ref Summary summary, int depth, bool whiteToMove)
        {
            if (whiteToMove)
            {
                ...
                while (positions != 0)
                {
                    PerftWhitePawn(ref board, ref summary, depth, positions.PopLSB());
                }

                positions = board.WhiteKnight;
                while (positions != 0)
                {
                    PerftWhiteKnight(ref board, ref summary, depth, positions.PopLSB());
                }

                positions = board.WhiteBishop;
                while (positions != 0)
                {
                    PerftWhiteBishop(ref board, ref summary, depth, positions.PopLSB());
                }

                positions = board.WhiteRook;
                while (positions != 0)
                {
                    PerftWhiteRook(ref board, ref summary, depth, positions.PopLSB());
                }

                positions = board.WhiteQueen;
                while (positions != 0)
                {
                    PerftWhiteQueen(ref board, ref summary, depth, positions.PopLSB());
                }

                positions = board.WhiteKing;
                while (positions != 0)
                {
                    PerftWhiteKing(ref board, ref summary, depth, positions.PopLSB());
                }

                if (oldNodes == summary.Nodes)
                {
                    summary.CheckMates++;
                }
            }
            else
            {
                ...
                while (positions != 0)
                {
                    PerftBlackPawn(ref board, ref summary, depth, positions.PopLSB());
                }

                positions = board.BlackKnight;
                while (positions != 0)
                {
                    PerftBlackKnight(ref board, ref summary, depth, positions.PopLSB());
                }

                positions = board.BlackBishop;
                while (positions != 0)
                {
                    PerftBlackBishop(ref board, ref summary, depth, positions.PopLSB());
                }

                positions = board.BlackRook;
                while (positions != 0)
                {
                    PerftBlackRook(ref board, ref summary, depth, positions.PopLSB());
                }

                positions = board.BlackQueen;
                while (positions != 0)
                {
                    PerftBlackQueen(ref board, ref summary, depth, positions.PopLSB());
                }

                positions = board.BlackKing;
                while (positions != 0)
                {
                    PerftBlackKing(ref board, ref summary, depth, positions.PopLSB());
                }

                if (depth == 1 && oldNodes == summary.Nodes)
                {
                    summary.CheckMates++;
                }
            }
